...
public class MyController {
    ...
    public List<Value> run(RestTemplate restTemplate) throws Exception {
        ...
        // log.info(Arrays.stream(objects).map(object -> mapper.convertValue(object,
        // Value.class))
        // .collect(Collectors.toList()).toString());
        Plane = Arrays.stream(objects).map(object -> mapper.convertValue(object, Value.class))
    ...
    @Bean
    public List<Value> jpafunct(PlaneRepo repository) {
            for (int i = 0; i < Plane.size(); i++) {
                repository.save(Plane.get(i));
            }
            Value plane = repository.findById(1L);
    ...
    @Bean
    @RequestMapping("/testejpa")
    public String somePage2(PlaneRepo repository) {
        Value plane = repository.findById(1L);
        String plane1 = plane.toString();
        log.info("estou aqui");
        log.info(plane1);
        return plane1;
    }
    ...
    public ModelAndView getBlog(ModelAndView mv) {
        mv.addObject("planes", Plane);
        mv.setViewName("filterpage");
